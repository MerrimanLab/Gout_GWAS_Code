...
urate<-read.delim("/Volumes/biochemistry/Lab_Groups/merrimanlab/Merriman_Documents/Ruth/UKBioBank/Phenotypes/data/ukb_urate_umolL_v0.txt", stringsAsFactors = F)
...
goutprs<-read.delim("/Volumes/biochemistry/Lab_Groups/merrimanlab/Merriman_Documents/Ruth/2021_projects/post_GWAS_analysis/PRS/UKBB_tin114_grs_2021_for_PRS.txt")
...
pheno2$GoutPRS_bins[which(pheno2$GoutPRS_bins=="[3.56,3.85)")]=NA
pheno2$GoutPRS_bins<-sub("4.71,5" ,"4.71,5.00",pheno2$GoutPRS_bins)
pheno2$GoutPRS_bins<-sub("5,5.29","5.00,5.29",pheno2$GoutPRS_bins)

resulttable <- as.data.frame(matrix(ncol=6, nrow=0))
colnames(resulttable)<-c("name","OR","LCI","UCI", "P","N" )
pheno2$GoutPRS_bins<-factor(pheno2$GoutPRS_bins, levels =c("[4.71,5.00)","[3.85,4.14)", "[4.14,4.42)", "[4.42,4.71)", "[5.00,5.29)", "[5.29,5.58)", "[5.58,5.87)", "[5.87,6.16)", "[6.16,6.45]"  ))
test<-glm(gout_AFFSTAT~GoutPRS_bins+AGE+SEX, data=pheno2, family = 'binomial') 

for(b in seq(2:12)){
    resulttable[b,]<-results(test=test,i=b)
}
resulttable[1,]<-c("[4.71,5.00)","1","1","1",NA, NA)
resulttable[,1]<-c("[4.71,5.00)", "[3.85,4.14)", "[4.14,4.42)", "[4.42,4.71)", "[5.00,5.29)", "[5.29,5.58)", "[5.58,5.87)", "[5.87,6.16)", "[6.16,6.45]" ,  "Age", "Sex")
...
resulttable$name <- factor(resulttable$name, levels = c("[3.85,4.14)", "[4.14,4.42)", "[4.42,4.71)", "[4.71,5.00)", "[5.00,5.29)", "[5.29,5.58)", "[5.58,5.87)", "[5.87,6.16)", "[6.16,6.45]", "Age", "Sex"))
...
#full unadjusted
auc(affstat ~ GoutPRS, data=test_dataset)   #Compute AUC for predicting case with the variable age
#full adjusted
mod1<-glm(affstat ~ GoutPRS+ AGE+as.factor(SEX), data=test_dataset, family="binomial")  #Logistic regression model
auc(affstat  ~ predict(mod1), data=test_dataset)  #Compute AUC for predicting case with your model
...
auc(affstat ~ GoutPRS, data=test_dataset_males)   #Compute AUC for predicting case with the variable age
#full adjusted
mod1<-glm(affstat ~ GoutPRS+ AGE, data=test_dataset_males, family="binomial")  #Logistic regression model
auc(affstat  ~ predict(mod1), data=test_dataset_males)  #Compute AUC for predicting case with your model


#females
test_dataset_females<-test_dataset[which(test_dataset$SEX==0),]
auc(affstat ~ GoutPRS, data=test_dataset_females)   #Compute AUC for predicting case with the variable age
#full adjusted
mod1<-glm(affstat ~ GoutPRS+ AGE, data=test_dataset_females, family="binomial")  #Logistic regression model
auc(affstat  ~ predict(mod1), data=test_dataset_females)  #Compute AUC for predicting case with your model
...
males$GoutPRS_bins[which(males$GoutPRS_bins=="[3.56,3.85)")]=NA
males$GoutPRS_bins<-sub("4.71,5" ,"4.71,5.00",males$GoutPRS_bins)
males$GoutPRS_bins<-sub("5,5.29","5.00,5.29",males$GoutPRS_bins)

resulttable_males <- as.data.frame(matrix(ncol=6, nrow=0))
colnames(resulttable_males)<-c("name","OR","LCI","UCI", "P","N" )
males$GoutPRS_bins<-factor(males$GoutPRS_bins, levels =c("[4.71,5.00)","[3.85,4.14)", "[4.14,4.42)", "[4.42,4.71)", "[5.00,5.29)", "[5.29,5.58)", "[5.58,5.87)", "[5.87,6.16)", "[6.16,6.45]" ))
test<-glm(gout_AFFSTAT~GoutPRS_bins+AGE,data=males) 

for(b in seq(1:10)){
    resulttable_males[b,]<-results(test=test,i=b)
}
resulttable_males[1,]<-c("[4.71,5.00)","1","1","1",NA, NA)
resulttable_males[,1]<-c("[4.71,5.00)","[3.85,4.14)", "[4.14,4.42)", "[4.42,4.71)", "[5.00,5.29)", "[5.29,5.58)", "[5.58,5.87)", "[5.87,6.16)", "[6.16,6.45]", "Age")
...
resulttable_males$name <- factor(resulttable_males$name, levels = c("[3.85,4.14)", "[4.14,4.42)", "[4.42,4.71)","[4.71,5.00)", "[5.00,5.29)", "[5.29,5.58)", "[5.58,5.87)", "[5.87,6.16)", "[6.16,6.45]", "Age"))
...
pROC::roc(response = test_dataset_males$affstat, test_dataset_males$GoutPRS, plot=TRUE, legacy.axes=TRUE, percent=TRUE, xlab="False Positive Percentage", ylab="True Postive Percentage")
...
sumfilegout_females$controls<-table(females$GoutPRS_bins, females$gout_AFFSTAT)[1:8]
sumfilegout_females$gout<-table(females$GoutPRS_bins, females$gout_AFFSTAT)[9:16]
...
test<-glm(gout_AFFSTAT~GoutPRS_bins+AGE,data=females) 
...
pROC::roc(response = test_dataset_females$affstat, test_dataset_females$GoutPRS, plot=TRUE, legacy.axes=TRUE, percent=TRUE, xlab="False Positive Percentage", ylab="True Postive Percentage")
```
